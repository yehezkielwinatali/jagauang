...
interface UpdateDefaultAccountResult {
  success?: boolean;
  error?: string;
...
interface AccountWithTransactions extends SerializedAccount {
...
export async function updateDefaultAccount(
  accountId: string
): Promise<UpdateDefaultAccountResult | void> {
  try {
    const { userId }: { userId: string | null } = await auth();
    if (!userId) throw new Error("Unauthorized");
    const user: { id: string; clerkUserId: string } | null =
...
export async function bulkDeleteTransactions(
  ...
  try {
    ...
    revalidatePath("/dashboard");
    revalidatePath("/account/[id : string]");
